...
// Se la proprietà done è uguale a true, visualizzare 
// il testo del todo sbarrato.
    ...
    data() {
        return {
            todos: [
                ...
                {
                    text: 'Portare a passeggio il pappagallo',
                    done: false
                },
            ],
            newTodoText: ''
    ...
    methods: {
        ...
        addNewTodo() {
            if(this.newTodoText.length > 0) {
                // Devo aggiungere un nuovo todo all'array
                // Creare un nuovo oggetto con text e done
                const newTodo = {
                    text: this.newTodoText,
                    done: false
                };

                // Aggiungere il nuovo oggetto a this.todos
                this.todos.unshift(newTodo);
                this.newTodoText = '';
            }
        },
        toggleDone(indexToChange) {
            // Cambiare il done dell'elemento con indice indexToChange
            // if(this.todos[indexToChange].done === true) {
            //     this.todos[indexToChange].done = false;
            // } else {
            //     this.todos[indexToChange].done = true;
            // }

            // this.todos[indexToChange].done = this.todos[indexToChange].done === true ? false : true;
